...
module.exports = async (req, res) => {
  console.log("deleteAGPTRecords called");
  try {
    let indexes = [];
    ...
    if (indexes.length > 1) {
      ...
      for (let i = 0; i < newIndexes.length; i++) {
        let collectionRef = db.collection("AggregatedGenerationPerType" + newIndexes[i]);
    ...
    // Sends error message
    console.log("error in deleteAGPTRecords is:", e);
  }
};
